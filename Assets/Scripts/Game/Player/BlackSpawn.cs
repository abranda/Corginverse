...
public class BlackSpawn : MonoBehaviour {
	...
	public BlackCollisionDetection bottom;
	GameObject spawned;
	bool youWon;
	...
	void Update(){
		...
		if (spawned != null) {
			...
			if (spawned.GetComponent<Animator> ().GetBool ("Ended")) {
				...
				GetComponent<BlackMovement> ().canMove = true;

				GameObject temp = spawned;

				spawned = (GameObject)Instantiate (spawnable);
				spawned.GetComponent<WhiteSpawn> ().CustomMap = CustomMap;
				...
				GetComponent<BlackMovement> ().canMove = true;
				GetComponent<BlackMovement> ().canMoveLeft = true;
				GetComponent<BlackMovement> ().canMoveRight = true;
				this.enabled = false;
	...
	void OnGUI(){
		...
		if (GetComponent<InFocus> ().isFocus && !youWon) {
			...
			if (bottom.grey || GetComponent<BlackMovement> ().left || GetComponent<BlackMovement> ().right || (GetComponent<BlackMovement> ().jumping || GetComponent<BlackMovement> ().falling)) {
				canSpawn = false;
				if (focusArrow != null) {
			...
			if (canSpawn && !GetComponent<BlackMovement> ().jumping && !GetComponent<BlackMovement> ().falling && !spawnLimit) {
			...
			if (CustomMap) {
				if (GameObject.FindGameObjectWithTag ("EditorObject") != null) {
					...
					if (GUI.Button (new Rect ((Screen.width / 4) + (Screen.width / 2), 0, Screen.width / 4, Screen.height / 4), "Back to Menu", customButton)) {
						UnityEngine.SceneManagement.SceneManager.LoadScene (2);
					}
				}
			} else {
